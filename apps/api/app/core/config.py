...
FEATURE_FLAG_NAMES = (
    ...
    "AI_ANALYSIS_SOURCE_FALLBACK_ENABLED",
    ...
    "MANAGED_STORAGE_AUTO_CLEANUP_ENABLED",
...
class Settings(BaseSettings):
    ...
    # Read-only, tenant-scoped fallback when managed storage auth is permanently unavailable.
    AI_ANALYSIS_SOURCE_FALLBACK_ENABLED: bool = False
    ...
    # Explicit opt-in prevents cleanup from deleting an existing backlog on deploy.
    MANAGED_STORAGE_AUTO_CLEANUP_ENABLED: bool = False
    ...
    DOLA_RENDER_GATEWAY_ENABLED: bool = False
    VIDEO_GENERATION_CANARY_TENANT_IDS: str = ""
    DOLA_RENDER_GATEWAY_URL: str = "http://127.0.0.1:8100"
    DOLA_RENDER_GATEWAY_INTERNAL_KEY: str = ""
    ...
    VIDEO_PROXY_VIDEO_BITRATE_KBPS: int = 1500
    VIDEO_PROXY_AUDIO_BITRATE_KBPS: int = 64
    ...
    # CP-13: deny-by-default daily discovery canary.
    CREATIVE_PIPELINE_CANARY_ENABLED: bool = False
    CREATIVE_PIPELINE_CANARY_ROOT_FOLDER_ID: str = ""
    CREATIVE_PIPELINE_CANARY_TIMEZONE: str = "Asia/Ho_Chi_Minh"
    CREATIVE_PIPELINE_CANARY_SCAN_HOUR: int = 6
    CREATIVE_PIPELINE_CANARY_POLL_INTERVAL_SECONDS: float = 60.0
    CREATIVE_PIPELINE_CANARY_MAX_ACTIVE_RUNS: int = 1
    # CP-16: execution stays disabled until all exact canary scopes are configured.
    CREATIVE_PIPELINE_ROLLOUT_ENABLED: bool = False
    CREATIVE_PIPELINE_ROLLOUT_TENANT_IDS: str = ""
    CREATIVE_PIPELINE_ROLLOUT_EXTERNAL_SOURCE_IDS: str = ""
    CREATIVE_PIPELINE_ROLLOUT_SOURCE_GROUP_FOLDER_IDS: str = ""
    CREATIVE_PIPELINE_ROLLOUT_LISTING_FOLDER_IDS: str = ""
    CREATIVE_PIPELINE_ROLLOUT_MAX_ACTIVE_RUNS: int = 1
    ...
    MANAGED_STORAGE_COMPLETED_RETENTION_HOURS: int = 6
    MANAGED_STORAGE_FAILED_RETENTION_HOURS: int = 24
    ...
    def validate_worker_runtime(self) -> "Settings":
        ...
        if min(self.MANAGED_STORAGE_COMPLETED_RETENTION_HOURS, self.MANAGED_STORAGE_FAILED_RETENTION_HOURS) < 0:
            raise ValueError("Managed storage retention hours cannot be negative")
        if min(self.MANAGED_STORAGE_CLEANUP_INTERVAL_SECONDS, self.MANAGED_STORAGE_CLEANUP_BATCH_SIZE, self.MANAGED_STORAGE_CLEANUP_MAX_ITEMS_PER_RUN) <= 0:
            raise ValueError("Managed storage cleanup limits must be positive")
        if self.MANAGED_STORAGE_CLEANUP_INTERVAL_SECONDS < 60:
