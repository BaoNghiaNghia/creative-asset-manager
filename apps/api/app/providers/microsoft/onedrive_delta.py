...
def _candidate(item,source_id,drive_id):
 ...
 metadata={"drive_id":item_drive,"item_id":item.get("id"),"parent_drive_id":parent.get("driveId"),"parent_item_id":parent.get("id"),"is_folder":folder,"web_url":item.get("webUrl"),"etag":item.get("eTag")}
 duration=(item.get("video") or {}).get("duration")
 if isinstance(duration,(int,float)) and not isinstance(duration,bool) and duration >= 0: metadata["video_duration_ms"]=int(duration)
 return ExternalAssetCandidate(source_type="onedrive",source_id=source_id,external_asset_id=make_item_id(item_drive,str(item["id"])),filename=item.get("name"),mime_type=FOLDER_MIME if folder else file_data.get("mimeType"),size_bytes=item.get("size"),source_created_at=item.get("createdDateTime"),source_modified_at=item.get("lastModifiedDateTime"),provider_checksum=hashes.get("sha256Hash") or hashes.get("sha1Hash"),provider_version=item.get("eTag") or item.get("cTag"),source_metadata=metadata)
async def list_onedrive_delta(access_token,input):
 ...
 url=validate_graph_url(input.cursor) if input.cursor else f"https://graph.microsoft.com/v1.0/drives/{drive_id}/root/delta";params=None if input.cursor else {"$top":str(input.page_size),"$select":"id,name,size,createdDateTime,lastModifiedDateTime,webUrl,parentReference,file,folder,video,deleted,eTag,cTag"}
