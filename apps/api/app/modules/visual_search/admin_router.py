...
dashboard_cache = BoundedTTLCache[tuple[str], dict](max_entries=128, ttl_seconds=45)

def _invalidate_dashboard(tenant_id: str):
 dashboard_cache.invalidate_where(lambda key: key[0] == tenant_id)

class SliceRequest(BaseModel): max_assets:int=100
def service():
 s=get_settings();return VisualCoverageService(SessionLocal,VisualSearchElasticsearchIndex(ElasticsearchV3Config(s.ELASTICSEARCH_URL,s.ELASTICSEARCH_INDEX_PREFIX,index_generation="v3"),__import__("app.modules.visual_search.model_spec",fromlist=["VISUAL_SEARCH_ACTIVE_DESCRIPTOR"]).VISUAL_SEARCH_ACTIVE_DESCRIPTOR))
@router.get("/coverage")
...
def coverage_dashboard(principal:CurrentPrincipal=Depends(READ)):
 key=(principal.active_tenant_id,)
 cached=dashboard_cache.get(key)
 if cached is not None:return deepcopy(cached)
 r=service().collect(principal.active_tenant_id)
 result={"generated_at":datetime.now(timezone.utc),"index_state":r.index_state,"totals":r.totals,"ratios":r.ratios,"sources":r.sources}
 dashboard_cache.put(key,deepcopy(result))
 return result
...
def start_backfill(principal:CurrentPrincipal=Depends(WRITE)):
 run=_run(principal.active_tenant_id,principal); _invalidate_dashboard(principal.active_tenant_id); return {"id":run.id,"status":run.status,"checkpoint_asset_id":run.checkpoint_asset_id}
...
def pause_backfill(run_id:str,principal:CurrentPrincipal=Depends(WRITE)):
 with SessionLocal() as session:
  ...
  session.commit(); _invalidate_dashboard(principal.active_tenant_id); return {"id":run.id,"status":run.status}
...
def run_backfill_slice(run_id:str,body:SliceRequest,principal:CurrentPrincipal=Depends(WRITE)):
 ...
 with SessionLocal() as session:
  index=VisualSearchElasticsearchIndex(ElasticsearchV3Config(settings.ELASTICSEARCH_URL,settings.ELASTICSEARCH_INDEX_PREFIX,index_generation="v3"),__import__("app.modules.visual_search.model_spec",fromlist=["VISUAL_SEARCH_ACTIVE_DESCRIPTOR"]).VISUAL_SEARCH_ACTIVE_DESCRIPTOR)
  ...
  session.commit(); _invalidate_dashboard(principal.active_tenant_id); return {"id":run.id,"status":run.status,"checkpoint_asset_id":run.checkpoint_asset_id,"result":result.__dict__ if result else None}
