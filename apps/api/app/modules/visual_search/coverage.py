...
def _ok(doc,resource):
 d=VISUAL_SEARCH_ACTIVE_DESCRIPTOR
...
class VisualCoverageService:
 ...
 def collect(self,tenant_id):
  ...
  job_assets={r.asset_id:r.content_hash for r in resources if r.eligible and r.asset_id}
  ...
  totals={k:sum(b[k] for b in buckets.values()) for k in KEYS}
  current_ids={r.asset_id for r in resources if r.eligible and any(_ok(d,r) for d in by[r.asset_id])}
  with self.session_factory() as s: totals.update(VisualJobCoverageReader(s).counts(tenant_id,job_assets,current_ids))
