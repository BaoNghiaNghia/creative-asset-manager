...
class VisualCoverageResourceReader:
    ...
    def resources(self, tenant_id:str)->list[CoverageResource]:
        ...
        for source,link,asset,external in rows:
            ...
            result.append(CoverageResource(source.id,source.external_source_id,external.source_type,external.display_name,source.mime_type,asset.id if imported else None,asset.content_hash if imported else None,imported,imported and supported and valid_hash,not supported))
        return result
