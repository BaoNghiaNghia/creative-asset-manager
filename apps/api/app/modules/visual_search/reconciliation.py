...
class VisualSearchReconciliationService:
    ...
    def reconcile(self, *, tenant_id: str, max_assets: int=100, after_asset_id: str | None=None) -> VisualReconciliationResult:
        ...
        candidates=[asset_id for asset_id in sorted(assets) if after_asset_id is None or asset_id > after_asset_id]
        batch=candidates[:max_assets]
        ...
        return VisualReconciliationResult(len(batch),current,missing,stale,enqueued,existing,batch[-1] if batch else after_asset_id,len(candidates)>len(batch))
