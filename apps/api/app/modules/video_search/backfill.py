...
from app.modules.video_search.enqueue import (
    enqueue_video_analysis_job,
    video_enqueue_identity,
    video_enqueue_job_exists,
)
...
class VideoAnalysisBackfillService:
    ...
    def run(self, *, tenant_id: str, source_asset_id: str | None = None, limit: int = 100, batch_size: int = 100, dry_run: bool = False) -> VideoBackfillResult:
        if not tenant_id:
        ...
        while result.scanned < limit:
            ...
            if after_id is not None:
                page = page.where(SourceAssetModel.id > after_id)
            assets = self.session.scalars(
                page.order_by(SourceAssetModel.id.asc()).limit(
                    min(batch_size, limit - result.scanned)
                )
            ).all()
            if not assets:
                break
            for asset in assets:
                ...
                if profiles.find_resumable_compatible_run(**identity) is not None:
                    ...
                    continue
                enqueue_identity = video_enqueue_identity(
                    tenant_id=tenant_id, source_asset=asset,
                    processing=self.processing, settings=self.settings,
                )
                if enqueue_identity is None or video_enqueue_job_exists(
                    processing=self.processing, identity=enqueue_identity
                ):
                    result.skipped_existing_job += 1
                    continue
                if dry_run:
