...
def enqueue_video_search_index_job(*, tenant_id: str, run: VideoAnalysisRunModel, processing: ProcessingRepository) -> bool:
    ...
    _job, created=processing.create_job_once(tenant_id=tenant_id,job_type="video_search_index",entity_type="video_analysis_run",entity_id=run.id,idempotency_key=key,payload={"analysis_run_id":run.id},priority=VIDEO_SEARCH_INDEX_PRIORITY,provider_key="elasticsearch",provider_scope="video")
    return created
