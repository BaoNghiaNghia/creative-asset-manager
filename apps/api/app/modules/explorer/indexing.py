...
def _job_key(account_id: str, provider: str) -> str:
    return f"{provider}:{account_id}"


def get_index_status(account_id: str, provider: str = "google-drive") -> IndexStatus:
    return _jobs.get(_job_key(account_id, provider), IndexStatus())
...
def start_index_job(
    ...
    key = _job_key(account_id, body.provider)
    current_task = _tasks.get(key)
    if current_task and not current_task.done():
        return get_index_status(account_id, body.provider)

    source_name = "SharePoint" if body.provider == "sharepoint" else "Google Drive"
    job = IndexStatus(
        state="running",
        status=f"Preparing {source_name} metadata index",
        progress=1,
    ...
    async def run():
        try:
            ...
            job.status = (
                ...
                else f"{source_name} metadata is ready"
            )
            job.progress = 100
            ...
            logger.exception("%s metadata indexing failed for the current account", source_name)
            job.state = "failed"
