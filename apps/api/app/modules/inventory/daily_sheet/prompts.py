...
BUILTIN_PROMPTS = {
    "daily_gemini_processing": """Process today's Gemini working workbook. Use the sheet \"Báo cáo sử dụng NVL trong ca\" as operational evidence and reconcile its daily material data into the relevant inventory/location sheets where those materials belong. Discover relationships from workbook metadata, headers, merged cells, material/location catalogs and exact evidence rather than fixed sheet positions or columns. Preserve independent quantity dimensions and do not invent package, recipe or BOM conversions. Do not map a material by row number alone. In automatic mode, process each report material independently: update only an existing editable warehouse row with an exact normalized name or confirmed catalog alias; a close name needs compatible category, unit and operational-context evidence from both rows. Leave unmatched materials unchanged, record UNMAPPED_MATERIAL_SKIPPED as an informational issue, and continue other evidence-backed material operations. Stage only evidence-backed edits, preserve formulas and structure, and report ambiguity instead of guessing. The goal is that today's Gemini workbook becomes the verified end-of-day inventory result.""",
    "carry_forward_0900": """Initialize the current shared workbook for today's business date using the previous day's verified final Gemini workbook. Understand both workbooks dynamically from metadata, headers, merged headers, formulas, values, material/location catalogs and exact cell evidence. Identify yesterday's terminal inventory state and today's corresponding starting state. Also identify daily operational input fields that must be cleared or reset. Do not assume fixed sheet positions, rows, columns or labels. Do not modify formulas, identities, units, structural cells or unrelated fields. Every set or clear operation must cite exact evidence. If a reset rule or source-to-target relationship is ambiguous, report it instead of guessing.""",
}
