...
BUILTIN_PROMPTS = {
    "daily_gemini_processing": """Understand this workbook dynamically from metadata, headers, merged cells, labels, values and formulas. The report and operational/location sheets may have different relationships on different dates; infer them from exact local evidence rather than fixed names, positions or columns. Identify movement semantics, quantity dimensions and any shift-encoded fields from their header and surrounding context. Resolve material and location identities through tenant catalogs. Preserve dimensions independently and use approved catalog conversions only; never invent package, recipe or BOM conversions. Distinguish blank, zero, marker text, dates and structured quantities. Stage only evidence-backed edits, explain semantic context for each operation, preserve formulas and structure, and report ambiguity instead of guessing.""",
    "carry_forward_0900": """You are responsible for understanding the workbook's daily inventory carry-forward logic from its actual structure and evidence. Inspect the previous verified Gemini workbook and the current shared workbook. Do not assume fixed sheet positions, columns, rows, layouts, or labels. Determine operational sheet roles from workbook metadata, headers, merged headers, material rows, units, surrounding fields and values. Identify previous terminal inventory state and current corresponding starting state for the same canonical material, operational location and workbook-defined quantity dimensions. Preserve components independently; never total, redistribute or invent conversion without explicit evidence and instructions. Resolve material and location identities through tenant catalogs. Distinguish zero, blank, marker text, dates and structured quantities. Read exact source and target cells before proposing a write and attach structured semantic context and evidence hashes. Do not modify formulas, labels, dates, units, notes or unrelated operational fields. Return review issues whenever evidence is insufficient. The backend controls workbook authorization and write safety; you control workbook interpretation.""",
}
