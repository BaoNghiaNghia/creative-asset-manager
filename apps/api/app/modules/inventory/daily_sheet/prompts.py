...
class InventoryPromptResolver:
    ...
    def resolve(self, tenant_id: str, prompt_type: str, *, legacy_goals: list[str] | None = None) -> ResolvedInventoryPrompt:
        ...
        try:
            ...
            # Test/legacy databases created before migration 0071 retain the
            # documented built-in or V4 legacy fallback until migrated.
            row = None
    ...
    def draft(self, tenant_id: str, prompt_type: str, content: str, actor: str | None):
        ...
        with self.sessions() as session:
            ...
            session.add(row); session.commit(); session.refresh(row); return self._view(row)
    def activate(self, tenant_id: str, prompt_type: str, prompt_id: str, actor: str | None):
        ...
        with self.sessions() as session:
            ...
            row.status="active"; row.activated_by=actor; row.activated_at=now; session.commit(); session.refresh(row); return self._view(row)
