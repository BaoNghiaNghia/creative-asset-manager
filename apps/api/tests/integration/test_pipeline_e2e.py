...
class DurablePipelineEndToEndTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.settings=Settings(PROCESSING_JOBS_ENABLED=True,UNIFIED_ASSET_INGESTION_ENABLED=True,CONTENT_DEDUP_ENABLED=True,MANAGED_ASSET_STORAGE_ENABLED=True,DYNAMIC_AI_METADATA_ENABLED=True,AI_SINGLE_ANALYSIS_ENABLED=True,AI_AUTO_ANALYZE_ENABLED=True,SEARCH_PROJECTION_ENABLED=True,ELASTICSEARCH_V2_ENABLED=True,SEARCH_QUERY_PARSER_V2_ENABLED=True,EXTERNAL_INGESTION_API_ENABLED=True,GEMINI_API_KEY="fake-no-network-key",GEMINI_MODEL="fake-gemini-v1",WORKER_LEASE_SECONDS=10,WORKER_HEARTBEAT_SECONDS=.2,WORKER_IDLE_POLL_SECONDS=.01)
