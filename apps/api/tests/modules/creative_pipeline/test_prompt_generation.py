...
def test_prompt_assembly_is_deterministic_and_durable():
    ...
    durable = durable_prompt(provider="seedance", model="seedance-1", platform="etsy", idea_story_version=1, snapshot_id="snap-1", draft=validate_draft({"prompt_draft_schema_version": 1, "outputs": [{"aspect_ratio": "1:1", "prompt": "x"}]}, profile))
