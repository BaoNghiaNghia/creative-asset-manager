...
class DatabaseBackupWorkflowTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_backup_follows_the_documented_workflow_order(self) -> None:
        events: list[str] = []
        local, directory = self._local()
        remote = _Remote()
        retention = _Retention()

        original_lock = local.exclusive_lock
        original_dump = local.create_verified_dump_locked
        original_cleanup = local.cleanup_verified_backup
        original_upload = remote.upload
        original_verify = remote.verify
        original_prune = retention.prune_after_verified_upload
        ...
        def ordered_dump() -> VerifiedDatabaseBackup:
            events.append("dump_and_local_verify")
            return original_dump()

        def ordered_cleanup(backup: VerifiedDatabaseBackup) -> None:
            events.append("local_cleanup")
            original_cleanup(backup)

        async def ordered_upload(backup: VerifiedDatabaseBackup):
            events.append("drive_upload")
        ...
        async def ordered_prune(storage, *, protected_remote_file_id: str):
            ...
            return await original_prune(
                storage, protected_remote_file_id=protected_remote_file_id)

        local.exclusive_lock = ordered_lock
        local.create_verified_dump_locked = ordered_dump
        local.cleanup_verified_backup = ordered_cleanup
        remote.upload = ordered_upload
        remote.verify = ordered_verify
        retention.prune_after_verified_upload = ordered_prune
