...
function ViewerFolderScopeEditor({ tenantId, membershipId, onMessage }: { tenantId: string; membershipId: string; onMessage: (value: string) => void }) {
  ...
  return <div className="viewer-folder-scope-backdrop">
    <div className="viewer-folder-scope-dialog" role="dialog" aria-modal="true" aria-labelledby="viewer-folder-title">
      ...
      <div className="viewer-folder-actions"><button type="button" className="viewer-folder-clear" disabled={!selected.size} onClick={() => setSelected(new Set())}>Clear selection</button><label>Reason <span aria-hidden="true">*</span><input value={reason} minLength={3} onChange={event => setReason(event.target.value)} placeholder="Explain this access change" /></label></div>
