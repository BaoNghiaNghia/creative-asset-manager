...
function MemberActions({ member, roles, tenantId, canManageMembers, canManageRoles, onDone, onMessage }: { member: AccessMember; roles: AccessRole[]; tenantId: string; canManageMembers: boolean; canManageRoles: boolean; onDone: () => void; onMessage: (value: string) => void }) {
  ...
  return <div className="member-actions">{canManageRoles && available.length > 0 && <div className="member-role-assignment"><label className="sr-only" htmlFor={`role-${member.membership_id}`}>Role for member</label><select id={`role-${member.membership_id}`} value={roleId} onChange={event => setRoleId(event.target.value)}><option value="">Assign role…</option>{available.map(role => <option key={role.id} value={role.id}>{role.name}</option>)}</select><button disabled={!roleId} onClick={async () => { await assignMemberRole(tenantId, member.membership_id, roleId, "Role assigned from Access Management"); onMessage("Role assigned."); onDone(); }}>Assign</button></div>}
    <div className="member-status-actions">{canManageMembers && transition && <button onClick={async () => { await updateMemberStatus(tenantId, member.membership_id, transition, `${transition} member from Access Management`); onMessage(`Member ${transition}d.`); onDone(); }}>{transition === "restore" ? "Restore" : "Activate"}</button>}
