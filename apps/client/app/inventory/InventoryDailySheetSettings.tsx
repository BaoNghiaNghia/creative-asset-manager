...
const emptyConfiguration: InventoryDailySheetConfiguration = {
  ...
  snapshot_time_local: "23:50",
  reconcile_time_local: "23:55",
  carry_forward_time_local: "05:00",
...
export function InventoryDailySheetSettings() {
  ...
  const [promptHistory, setPromptHistory] = useState<Record<string,Array<Record<string,unknown>>>>({});
  ...
  return <section className={"inventory-sheet-settings inventory-settings-redesign" + (isV4 ? " inventory-settings-v4" : "")}>
    ...
    <div className="inventory-settings-summary" aria-label="Tóm tắt cấu hình Inventory">
      ...
      <article><span>Lịch chạy</span><strong>{configuration.carry_forward_time_local} · {configuration.snapshot_time_local} · {configuration.reconcile_time_local}</strong><small>Reset đầu ngày · Snapshot · Đối soát</small></article>
    ...
    {isV4 ? <section className="inventory-settings-section"><div className="inventory-settings-section-heading"><div><h3>Gemini Prompts</h3><p>Chỉ thay đổi hướng dẫn nghiệp vụ. Quy tắc quyền file, material, warehouse, Closing → Opening, blank/zero, formula, evidence và phạm vi ghi vẫn được backend khóa.</p></div></div>
      {prompts.map((prompt)=><article key={prompt.prompt_type} className="inventory-prompt-card"><h4>{prompt.prompt_type === "daily_gemini_processing" ? "Xử lý file Gemini hằng ngày" : "Reset đầu ngày / Carry Forward"}</h4><small>Nguồn: {prompt.source} · Phiên bản: {prompt.version} · Hash: <span title={prompt.content_hash}>{prompt.content_hash.slice(0,8)}…</span></small>{prompt.source === "custom" ? <p>Custom prompt overrides legacy business_goal.</p> : prompt.prompt_type === "daily_gemini_processing" ? <p>business_goal có thể được dùng làm legacy fallback.</p> : null}<textarea value={promptDrafts[prompt.prompt_type]||""} onChange={(event)=>setPromptDrafts((current)=>({...current,[prompt.prompt_type]:event.target.value}))} rows={7}/><div className="inventory-settings-actions"><button type="button" disabled={busy} onClick={()=>void execute(()=>inventoryDailySheetApi.createPromptDraft(prompt.prompt_type,promptDrafts[prompt.prompt_type]||""),"Draft saved. It is not active yet.")}>Save Draft</button>{prompt.draft ? <button type="button" disabled={busy} onClick={()=>window.confirm("Activate this Gemini business prompt?") && void execute(()=>inventoryDailySheetApi.activatePrompt(prompt.prompt_type,prompt.draft!.id),"Prompt activated for the next run.")}>Activate</button> : null}<button type="button" disabled={busy} onClick={()=>void inventoryDailySheetApi.getPromptVersions(prompt.prompt_type).then((value)=>setPromptHistory((current)=>({...current,[prompt.prompt_type]:value.versions}))).catch((error)=>setMessage(error instanceof Error?error.message:"Unable to load prompt history."))}>History</button><button type="button" disabled={busy} onClick={()=>setPromptDrafts((current)=>({...current,[prompt.prompt_type]:prompt.active_content||prompt.builtin_content}))}>Discard local edit</button><button type="button" disabled={busy} onClick={()=>window.confirm("Reset custom prompt for future runs?") && void execute(()=>inventoryDailySheetApi.resetPrompt(prompt.prompt_type),"Custom prompt reset to fallback.")}>Reset to fallback</button></div>{(promptHistory[prompt.prompt_type]||[]).map((item)=><div key={String(item.id)} className="inventory-prompt-history"><small>v{String(item.version)} · {String(item.status)} · {String(item.content_hash).slice(0,8)}… · {String(item.created_at||"")}</small>{item.status !== "draft" ? <button type="button" disabled={busy} onClick={()=>window.confirm("Restore this version as a new active prompt?") && void execute(()=>inventoryDailySheetApi.restorePrompt(prompt.prompt_type,String(item.id)),"Historical prompt restored as a new version.")}>Restore</button> : null}</div>)}</article>)}
    </section> : null}

    {isV4 && (status?.last_snapshot?.prompt_hash || status?.carry_forward?.prompt_hash) ? <section className="inventory-settings-section"><h3>Prompt provenance</h3>{status?.last_snapshot?.prompt_hash ? <p>Daily Gemini: {status.last_snapshot.prompt_source} {status.last_snapshot.prompt_version} · <span title={status.last_snapshot.prompt_hash}>{status.last_snapshot.prompt_hash.slice(0,8)}…</span></p> : null}{status?.carry_forward?.prompt_hash ? <p>Reset đầu ngày / Carry Forward: {status.carry_forward.prompt_source} {status.carry_forward.prompt_version} · <span title={status.carry_forward.prompt_hash}>{status.carry_forward.prompt_hash.slice(0,8)}…</span></p> : null}</section> : null}
    ...
    <section className="inventory-settings-section">
      ...
      <div className="inventory-settings-form-grid inventory-settings-file-schedule-grid">
        ...
        <label>Giờ reset đầu ngày<input type="time" value={configuration.carry_forward_time_local} onChange={(event)=>update("carry_forward_time_local",event.target.value)}/><small>Ghi tồn đầu ngày hiện tại từ tồn cuối ngày trước đã được xác minh và reset các trường nhập hằng ngày theo prompt.</small></label>
    ...
    <section className="inventory-settings-section">
      ...
      <div className="inventory-sheet-status">
        ...
        <article><h3>Snapshot gần nhất</h3><dl><div><dt>Ngày</dt><dd>{status?.last_snapshot?.business_date || "—"}</dd></div><div><dt>Kết quả</dt><dd>{status?.last_snapshot?.status || "Chưa chạy"}</dd></div><div><dt>Original snapshot</dt><dd>{status?.last_snapshot?.snapshot_url ? <a href={status.last_snapshot.snapshot_url} target="_blank" rel="noreferrer">Mở file ↗</a> : "Chưa tạo"}</dd></div><div><dt>Lỗi</dt><dd>{status?.last_snapshot?.error_code || "Không có"}</dd></div></dl></article>
        <article><h3>Gemini working copy</h3><dl><div><dt>Trạng thái</dt><dd>{status?.last_snapshot?.gemini_file_id ? "Sẵn sàng" : status?.last_snapshot?.status === "completed" ? "Cần khôi phục" : "Chưa tạo"}</dd></div><div><dt>Ngày dữ liệu</dt><dd>{status?.last_snapshot?.business_date || "—"}</dd></div><div><dt>File Gemini</dt><dd>{status?.last_snapshot?.gemini_url ? <a href={status.last_snapshot.gemini_url} target="_blank" rel="noreferrer">Mở file ↗</a> : "Chưa có"}</dd></div><div><dt>Prompt</dt><dd>{status?.last_snapshot?.prompt_version || "Chờ snapshot"}</dd></div></dl></article>
