    ...
    <div className="inventory-prompt-grid">
      {prompts.map(prompt => {
        ...
        return <article key={prompt.prompt_type} className="inventory-prompt-editor-card">
          <header><div><span className="inventory-prompt-step">{prompt.prompt_type === "carry_forward_0900" ? "RESET" : "DAILY"}</span><h3>{labelFor(prompt.prompt_type)}</h3><p>{descriptionFor(prompt.prompt_type)}</p></div><span className={prompt.source === "custom" ? "inventory-prompt-source custom" : "inventory-prompt-source"}>{sourceLabel}</span></header>
