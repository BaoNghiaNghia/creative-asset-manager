    ...
    <div className="inventory-prompt-grid">
      {prompts.map(prompt => {
        ...
        return <article key={prompt.prompt_type} className="inventory-prompt-editor-card">
          ...
          <div className="inventory-prompt-actions"><button type="button" disabled={busy || !current.trim()} onClick={() => void execute(() => inventoryDailySheetApi.createPromptDraft(prompt.prompt_type, current), "Đã lưu bản nháp. Prompt này chưa được áp dụng.")}>Lưu bản nháp</button>{prompt.draft ? <button type="button" className="secondary" disabled={busy} onClick={() => window.confirm("Kích hoạt bản nháp cho lần chạy Inventory tiếp theo?") && void execute(() => inventoryDailySheetApi.activatePrompt(prompt.prompt_type, prompt.draft!.id), "Đã kích hoạt prompt cho lần chạy tiếp theo.")}>Kích hoạt draft</button> : null}{prompt.prompt_type === "daily_gemini_processing" ? <button type="button" className="secondary" disabled={busy} onClick={() => { if (!window.confirm(`Chạy thử Gemini trên file _gemini hiện có. Prompt Active: ${prompt.version} · ${prompt.content_hash.slice(0, 12)}…`)) return; void execute(async () => { const result = await inventoryDailySheetApi.rerunCurrentGemini(); setRerunResult(result); }, "Đã hoàn tất chạy thử Gemini hôm nay."); }}>Chạy lại Gemini hôm nay</button> : null}<button type="button" className="text-button" disabled={busy} onClick={() => void loadHistory(prompt.prompt_type)}>{versions.length ? "Làm mới lịch sử" : "Xem lịch sử"}</button><button type="button" className="text-button" disabled={busy} onClick={() => setDrafts(value => ({ ...value, [prompt.prompt_type]: prompt.active_content || prompt.builtin_content }))}>Hoàn tác chỉnh sửa</button><button type="button" className="text-danger" disabled={busy || prompt.source !== "custom"} onClick={() => window.confirm("Đặt lại prompt tùy chỉnh về fallback?") && void execute(() => inventoryDailySheetApi.resetPrompt(prompt.prompt_type), "Đã đặt lại về prompt fallback.")}>Đặt lại fallback</button></div>
