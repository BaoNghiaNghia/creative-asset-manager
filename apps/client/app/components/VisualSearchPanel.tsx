...
export function VisualSearchPanel({ scope, canSearchAllResources, onScopeChange, hasCurrentSource, hasCurrentFolder, reference, loading, error, refinement, onRefinementChange, onUpload, onApplyCrop, onRetry, onClose }: Props) {
  ...
  return <section className="visual-search-panel" aria-label="Visual search">
    ...
    <fieldset className="visual-search-scope"><legend>Search scope</legend>{([ ["all", "All resources"], ["source", "Current source"], ["folder", "Current folder"] ] as const).map(([value, label]) => <label key={value}><input type="radio" name="visual-scope" value={value} checked={scope === value} disabled={value === "all" ? !canSearchAllResources : value === "source" ? !hasCurrentSource : !hasCurrentFolder} onChange={() => onScopeChange(value)} />{label}</label>)}</fieldset>{!scope && <div className="visual-search-context" role="status">Choose an authorized source or folder before searching.</div>}{!reference && <div className="visual-search-empty"><p>Choose an existing image or upload a reference image. Your upload is only used for this search.</p><button type="button" className="visual-primary" onClick={() => inputRef.current?.click()}>Upload reference image</button></div>}
