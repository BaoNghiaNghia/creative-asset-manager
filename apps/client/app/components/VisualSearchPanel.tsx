...
type Props = { reference: Reference; loading: boolean; error: string; onUpload: (file: File, crop?: VisualCrop) => void; onApplyCrop: (crop: VisualCrop) => void; onRetry: (crop?: VisualCrop) => void; onClose: () => void; };
const fullCrop: VisualCrop = { x: 0, y: 0, width: 1, height: 1 };

export function VisualSearchPanel({ reference, loading, error, onUpload, onApplyCrop, onRetry, onClose }: Props) {
  ...
  const preview = reference?.kind === "asset" ? assetPreviewUrl(reference.asset) : reference?.kind === "upload" ? reference.previewUrl : null;
