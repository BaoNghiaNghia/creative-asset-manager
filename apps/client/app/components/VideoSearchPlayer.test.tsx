...
describe("VideoSearchPlayer", () => {
  it("uses the existing scoped Explorer media stream rather than provider web URL", () => { const markup = renderToStaticMarkup(<VideoSearchPlayer item={item} onClose={() => {}} />); expect(markup).toContain("/api/explorer/media/external-a?provider=google-drive&amp;external_source_id=source-a"); expect(markup).not.toContain("https://drive.example/not-media"); expect(markup).toContain("controls"); expect(markup).toContain('aria-label="Close video player"'); expect(markup).toContain(">×</button>"); });
