...
export const api={
 exchange:(id:string,key:string)=>call(base(id)+"/session",{method:"POST",body:JSON.stringify({key})}), bootstrap:(id:string)=>call<Bootstrap>(base(id)+"/bootstrap"), folders:(id:string)=>call<{items:Folder[]}>(base(id)+"/folders"), children:(id:string,f:Folder,offset=0,limit=50)=>call<ChildPage>(base(id)+"/folders/"+encodeURIComponent(f.folder_id)+"/children?source_id="+encodeURIComponent(f.source_id)+"&offset="+offset+"&limit_value="+limit), search:(id:string,q:string)=>call<{items:Asset[]}>(base(id)+"/search?q="+encodeURIComponent(q)), annotations:(id:string,a:Asset)=>call<{items:Annotation[]}>(base(id)+"/assets/"+a.asset_id+"/annotations?source_asset_id="+a.source_asset_id),
 playbackTicket:(id:string,a:Asset)=>call<{url:string;cdn:boolean;expires_at:number|null}>(base(id)+"/assets/"+a.asset_id+"/playback-ticket?source_asset_id="+encodeURIComponent(a.source_asset_id)),
