...
export type PublicCommentActivity={annotation:Annotation;asset:Asset};
...
export const api={
 exchange:(id:string,key:string)=>call(base(id)+"/session",{method:"POST",body:JSON.stringify({key})}), bootstrap:(id:string)=>call<Bootstrap>(base(id)+"/bootstrap"), folders:(id:string)=>call<{items:Folder[]}>(base(id)+"/folders"), folder:(id:string,folderId:string)=>call<FolderResolution>(base(id)+"/folders/"+encodeURIComponent(folderId)), children:(id:string,f:Folder,offset=0,limit=50)=>call<ChildPage>(base(id)+"/folders/"+encodeURIComponent(f.folder_id)+"/children?source_id="+encodeURIComponent(f.source_id)+"&offset="+offset+"&limit_value="+limit), search:(id:string,q:string)=>call<{items:Asset[]}>(base(id)+"/search?q="+encodeURIComponent(q)), suggestions:(id:string,q:string)=>call<{suggestions:SearchSuggestion[]}>(base(id)+"/search/suggestions?q="+encodeURIComponent(q)+"&limit_value=10"), comments:(id:string,limit=100)=>call<{items:PublicCommentActivity[]}>(base(id)+"/comments?limit="+limit), annotations:(id:string,a:Asset)=>call<{items:Annotation[]}>(base(id)+"/assets/"+a.asset_id+"/annotations?source_asset_id="+a.source_asset_id),
 ...
 downloadUrl:(id:string,a:Asset)=>base(id)+"/assets/"+encodeURIComponent(a.asset_id)+"/download?source_asset_id="+encodeURIComponent(a.source_asset_id),
