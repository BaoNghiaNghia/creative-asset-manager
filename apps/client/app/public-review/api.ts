...
export const api={
 exchange:(id:string,key:string)=>call(base(id)+"/session",{method:"POST",body:JSON.stringify({key})}), bootstrap:(id:string)=>call<Bootstrap>(base(id)+"/bootstrap"), folders:(id:string)=>call<{items:Folder[]}>(base(id)+"/folders"), children:(id:string,f:Folder)=>call<{items:Child[]}>(base(id)+"/folders/"+encodeURIComponent(f.folder_id)+"/children?source_id="+encodeURIComponent(f.source_id)), search:(id:string,q:string)=>call<{items:Asset[]}>(base(id)+"/search?q="+encodeURIComponent(q)), annotations:(id:string,a:Asset)=>call<{items:Annotation[]}>(base(id)+"/assets/"+a.asset_id+"/annotations?source_asset_id="+a.source_asset_id),
