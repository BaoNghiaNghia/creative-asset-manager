...
import { PublicReviewHistory } from "./PublicReviewHistory";
...
export function PublicReviewRoute() {
 ...
 useEffect(() => { if (!historyOpen || current) return; const onKeyDown = (event: KeyboardEvent) => { if (event.key === "Escape") { event.preventDefault(); setHistoryOpen(false); } }; window.addEventListener("keydown", onKeyDown, true); return () => window.removeEventListener("keydown", onKeyDown, true); }, [historyOpen, current]);
 ...
 const toggleHistory = () => { if (!historyOpen) setViewHistory(readReviewHistory(id)); setHistoryOpen(value => !value); };
 const openHistoryEntry = (entry: ReviewHistoryEntry) => { const refreshed = readReviewHistory(id); const historyAssets = refreshed.map(historyEntryAsset); setHistoryOpen(false); selectMediaAsset(historyEntryAsset(entry), historyAssets, entry.folder_id); };
