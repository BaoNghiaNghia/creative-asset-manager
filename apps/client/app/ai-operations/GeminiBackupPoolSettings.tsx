...
export function GeminiBackupPoolSettings({ canManage = true, embedded = false }: { canManage?: boolean; embedded?: boolean }) {
  ...
  return <section className={"inventory-settings-card" + (embedded ? " inventory-settings-card-embedded" : "")}>
    <div className="inventory-settings-heading"><div><p className="inventory-kicker">FAILOVER AI</p><h2>Gemini Backup pool</h2><p className="inventory-muted">Round-robin across active Image backup keys. Configure up to 10 keys.</p></div></div>
    {items.map(item => <div key={item.slot} className="inventory-credential-grid" style={{ marginTop: 10 }}>
      ...
      {canManage && <div><input aria-label={`Backup ${item.slot} API key`} type="password" placeholder="New API key" value={drafts[item.slot]?.apiKey || ""} onChange={e => update(item.slot, "apiKey", e.target.value)} /><input aria-label={`Backup ${item.slot} label`} placeholder="Label" value={drafts[item.slot]?.label || ""} onChange={e => update(item.slot, "label", e.target.value)} /><button type="button" disabled={!drafts[item.slot]?.apiKey} onClick={() => void save(item.slot)}>{item.configured ? "Replace" : "Add key"}</button>{item.configured && <button type="button" className="danger" onClick={() => void remove(item.slot)}>Delete</button>}</div>}
    </div>)}
