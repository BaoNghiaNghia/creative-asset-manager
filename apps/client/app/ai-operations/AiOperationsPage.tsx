...
function VideoPipelineOverview({ dashboard, media, onMedia }: {
  ...
  return <div className="ops-content pipeline-content">
    ...
    <section className="pipeline-recent" aria-label="Tiến độ video gần đây"><div className="ops-table-heading"><div><h2>Tiến độ video gần đây</h2><p>Hiển thị tối đa 25 video được cập nhật gần nhất.</p></div></div>{dashboard.recent_video.length ? <div className="ops-table-scroll"><table className="ops-data-table"><thead><tr><th>Video</th><th>Phân tích AI</th><th>Lần thử</th><th>Cập nhật</th><th>Cần xử lý</th></tr></thead><tbody>{dashboard.recent_video.map(item => <tr key={item.job_id}><td>{item.filename || "Video " + item.source_asset_id.slice(0, 8)}</td><td><StatusText status={item.status} /></td><td>{item.attempt_count}/{item.max_attempts}</td><td>{new Date(item.updated_at).toLocaleString()}</td><td>{item.error_code || "—"}</td></tr>)}</tbody></table></div> : <p>Chưa có video analysis job nào.</p>}</section>
  </div>;
}
