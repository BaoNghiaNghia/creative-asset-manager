...
function SourcePlatform({ value, username }: { value?: string | null; username?: string | null }) {
  ...
  const logo = normalized === "google-drive" ? googleDrivePlatformLogo : normalized === "onedrive" ? oneDrivePlatformLogo : undefined;
  const accountLabel = username?.trim() || "—";
  return <span className={"source-platform " + normalized} title={username ? label + " (" + username + ")" : label}>
    {logo ? <img src={logo} alt="" aria-hidden="true" /> : <i aria-hidden="true">{normalized === "sharepoint" ? "S" : "?"}</i>}
    <span className="source-platform-copy"><strong>{label}</strong><small>{accountLabel}</small></span>
  </span>;
}
    ...
    {recent.items.length ? <div className="ops-table-scroll"><table className="ops-data-table">
      ...
      <tbody>{recent.items.map(item => {
        ...
        return <tr key={item.job_id}>
          ...
          <td><SourcePlatform value={item.source_type} username={item.source_username} /></td>
