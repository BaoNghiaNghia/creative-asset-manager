...
export function PipelineOverview({ pipeline, onPage = () => undefined }: { pipeline?: PipelineSnapshot | null; onPage?: (page: number, pageSize: 25 | 50 | 100) => void }) {
  ...
  return <div className="ops-content pipeline-content">
    ...
    {pipeline.skipped_breakdown?.length ? <details className="pipeline-exclusions"><summary><span><i aria-hidden="true">–</i><b>Excluded inputs</b><small>Permanent exclusions; no retry required</small></span><strong>{pipeline.skipped_breakdown.reduce((total, item) => total + item.count, 0).toLocaleString()}</strong></summary><p>These records are outside the image pipeline and do not count as failures.</p><ul>{pipeline.skipped_breakdown.map(item => <li key={item.category}><b>{item.category.replaceAll("_", " ")}</b><span>{item.count.toLocaleString()} input{item.count === 1 ? "" : "s"}</span></li>)}</ul></details> : null}
