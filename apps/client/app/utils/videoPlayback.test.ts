...
describe("video provider playback URL", () => {
  ...
  it("maps only supported provider types", () => { expect(playbackProvider("google_drive")).toBe("google-drive"); expect(playbackProvider("onedrive")).toBe("onedrive"); expect(playbackProvider("sharepoint")).toBe("sharepoint"); expect(buildVideoPlaybackUrl({ ...item, source_type: "unknown" })).toBeNull(); });
});
