export type DesktopIngestionStatus =
  | "scanning"
  | "hashing"
  | "duplicate"
  | "ready"
...
export interface DesktopIngestionItem {
  id: string;
  jobId: string;
  filename: string;
  relativePath: string;
  mimeType: string;
  size: number;
  status: DesktopIngestionStatus;
  bytesUploaded: number;
  attempts: number;
  errorCode?: string;
}

export interface DesktopIngestionJob {
  id: string;
  status: "scanning" | "ready" | "paused" | "completed" | "cancelled" | "failed";
  rootCount: number;
  discovered: number;
  supported: number;
  duplicates: number;
  ...
  skipped: number;
  uploading: number;
  items: DesktopIngestionItem[];
}

export interface DesktopNativeDragAsset {
  id: string;
  name: string;
  mimeType: string;
...
export interface DesktopNativeDragStats {
  count: number;
  cacheHits: number;
  cacheMisses: number;
  totalBytes: number;
...
export interface CamDesktopApi {
  ...
  beginOAuth(request: {
    ...
    intent?:
      ...
      | "onedrive_connect"
      | "onedrive_personal_connect"
      | "onedrive_work_connect";
    externalSourceId?: string;
  }): Promise<void>;
  ...
  ingestion: {
    ...
    snapshot(jobId: string): Promise<DesktopIngestionJob>;
